...
ลบบัญชีถาวร — ทำได้เฉพาะบัญชีที่ยังไม่เคยรับออเดอร์ แก้สต๊อก หรือส่งของ
บัญชีที่มีประวัติแล้วต้องกด "ระงับ" แทน และห้ามลบบัญชีตัวเองหรือผู้ดูแลระบบคนสุดท้าย
...
### DELETE /menu/items/:id/permanent 🔒 manager
**ลบเมนูถาวร** — ทำได้เฉพาะเมนูที่ยังไม่เคยถูกสั่ง ขนาด/ราคาทั้งหมดถูกลบตามไปด้วย
เมนูที่เคยขายแล้วจะได้ `409` พร้อมบอกว่าถูกสั่งไปกี่ครั้ง และแนะนำให้ปิดการขายแทน

### DELETE /menu/variants/:id 🔒 manager
ลบเฉพาะขนาด/ราคานั้น เมนูยังอยู่ — ขนาดที่เคยถูกสั่งแล้วลบไม่ได้

### DELETE /menu/toppings/:id 🔒 manager
ลบท็อปปิ้งถาวร — ท็อปปิ้งที่เคยถูกสั่งแล้วลบไม่ได้
...
ลบบิลถาวร — **ทำได้เฉพาะบิลที่ยกเลิกแล้ว** บิลที่ขายจริงลบไม่ได้ (`409`) เพราะยอดขายต้องตรวจย้อนหลังได้
รายการสินค้า ท็อปปิ้ง การชำระเงิน และประวัติการตัดสต๊อกของบิลนั้นถูกลบตามไปด้วย
...
### POST /orders/code/:code/notify-transfer
ลูกค้าแจ้งว่าโอนเงินแล้ว — เรียกได้โดยไม่ต้องล็อกอิน ใช้**รหัสบิล** (ไม่ใช่ id) เพราะรหัสสุ่มมา เดาไม่ได้จากการนับ

```json
// request — เลขอ้างอิงจากสลิป ไม่บังคับ
{ "reference": "1234" }

// response 200
{ "order_code": "PZ260923-4821", "total_amount": "359.00",
  "message": "แจ้งโอนเรียบร้อย ทางร้านจะตรวจสอบยอดเงินแล้วเริ่มทำอาหารให้" }
```

**ไม่ทำให้บิลกลายเป็น `paid`** แค่บันทึก `slip_notified_at` กับเลขอ้างอิงไว้ให้พนักงานเห็นในหน้าคิว
พนักงานต้องตรวจว่าเงินเข้าจริงแล้วกด `POST /orders/:id/pay` เอง จำกัด 10 ครั้ง / 10 นาที ต่อ IP

| กรณี | Code | ข้อความ |
|---|---|---|
| ไม่พบรหัสบิล | 404 | ไม่พบบิลนี้ กรุณาตรวจรหัสบิลอีกครั้ง |
| บิลถูกยกเลิก | 400 | บิลนี้ถูกยกเลิกแล้ว |
| จ่ายเงินแล้ว | 409 | บิลนี้ชำระเงินเรียบร้อยแล้ว |
...
ลบโค้ดถาวร — ทำได้เฉพาะโค้ดที่ยังไม่เคยถูกใช้กับออเดอร์ไหน โค้ดที่เคยใช้แล้วต้องกด "ปิด" แทน (`409`)
...
ลบวัตถุดิบถาวร — ทำได้เฉพาะตัวที่ยังไม่อยู่ในสูตรอาหารและไม่มีประวัติการเข้า-ออก (`409` พร้อมบอกว่าติดอะไรอยู่)
